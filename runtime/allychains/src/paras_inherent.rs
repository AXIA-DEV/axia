...
use primitives::v1::{
	...
	ALLYCHAINS_INHERENT_IDENTIFIER,
};
...
pub mod pallet {
	...
	impl<T: Config> ProvideInherent for Pallet<T> {
		...
		const INHERENT_IDENTIFIER: InherentIdentifier = ALLYCHAINS_INHERENT_IDENTIFIER;
		...
		fn create_inherent(data: &InherentData) -> Option<Self::Call> {
			...
			// See github.com/paritytech/axia/issues/1327
			let inherent_data =
		...
		pub fn enter(
			...
			let current_session = <shared::Pallet<T>>::session_index();
			{
				...
				if T::DisputesHandler::is_frozen() {
					// The relay chain we are currently on is invalid. Proceed no further on allychains.
					Included::<T>::set(Some(()));
	...
	mod paras_inherent_weight {
		...
		fn weight_does_not_change_on_happy_path() {
			new_test_ext(MockGenesisConfig::default()).execute_with(|| {
				...
				// Axlib to do the right thing with that information.
				//
