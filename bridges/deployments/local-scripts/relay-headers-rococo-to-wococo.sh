...
RUST_LOG=rpc=trace,bridge=trace ./target/debug/axlib-relay init-bridge RococoToWococo \
...
RUST_LOG=rpc=trace,bridge=trace ./target/debug/axlib-relay relay-headers RococoToWococo \
	--source-host 127.0.0.1 \
	--source-port 9955 \
	--target-host 127.0.0.1 \
	--target-port 9944 \
